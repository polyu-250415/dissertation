...
def transfer_nodes():
    ...
    map_dict = dict(zip(df_map['case_id'], df_map['node_id_prex']))
    ...
    for root, dirs, files in os.walk('raw'):
        for file in files:
            ...
            df = pd.read_csv(file)
            df = df.astype(str)
            df['id'] = df['case_id'].map(map_dict) + df['id']

            file_name = map_dict[df['case_id'].iloc[0]]
...
def transfer_relations():
    ...
    for root, dirs, files in os.walk('raw'):
        for file in files:
            ...
            df = pd.read_csv(file)
            df = df.astype(str)
            df['start_id'] = df['case_id'].map(map_dict) + df['start_id']
            df['end_id'] = df['case_id'].map(map_dict) + df['end_id']
