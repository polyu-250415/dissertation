...
class RebuildKG():
    ...
    def normalize_entities(self, redundant_id_dict, call_id):
        ...
        src_nodes_path = self.rag_ea_path + call_id + "_nodes.csv"
        dst_nodes_path = self.rebuild_kg_path + call_id + "_nodes.csv"
        src_relations_path = self.rag_ea_path + call_id + "_relations.csv"
        dst_relations_path = self.rebuild_kg_path + call_id + "_relations.csv"
        ...
        # 3. Process relations CSV
        relations_df = pd.read_csv(src_relations_path)
        # Replace src_node_id and dst_node_id where present in mapping
        ...
        relations_df.to_csv(dst_relations_path, index=False)
        print(f"Updated relations saved to {dst_relations_path}")
        print("Done.")

    def rebuild_all_kg(self, call_ids):
        ...
        for call_id in call_ids:
            self.normalize_entities(redundant_id_dict, call_id=call_id)
