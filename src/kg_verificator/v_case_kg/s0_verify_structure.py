...
class VerifySchema:
    ...
    def validate_nodes(self, case_id):
        ...
        err_output_path = f"{self.kg_case_path}{case_id}_nodes_r1_deleted.csv"
    ...
    def validate_relations(self, case_id):
        """
        Validate relation CSV against 13 relation rules.
        """
        input_path = f"{self.kg_case_path}{case_id}_relations.csv"
        valid_nodes_path = f"{self.kg_rag_path}{case_id}_nodes.csv"
        err_output_path = f"{self.kg_case_path}{case_id}_relations_error.csv"
        output_path = f"{self.kg_rag_path}{case_id}_relations.csv"
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(sc) or sc not in allow_src:
                df.loc[idx, "error_reason"] += f"Source must be {allow_src}; "
            if pd.isna(tc) or tc not in allow_dst:
                df.loc[idx, "error_reason"] += f"Target must be {allow_dst}; "
        ...
        if not invalid_df.empty:
            invalid_df.to_csv(err_output_path, index=False)
            print(f"Relation validation: {len(df)} total, {len(valid_df)} valid, {len(invalid_df)} invalid.")
        else:
    ...
    def validate_schema(self, case_ids):
        for case_id in case_ids:
            ...
            self.validate_relations(case_id)


if __name__ == "__main__":
    vs = VerifySchema()
    case_ids = ['c003']  # you can add more: ['c001', 'c002', 'c003']
