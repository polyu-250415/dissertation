...
from src.utils.llm_api import get_kimi_obj
...
class KimiAPI:
    ...
    @staticmethod
    def extract(prompt: str, result = []) -> dict:

        try:
            response = get_kimi_obj(prompt)
    ...
    def batch_extract(self, input_file, output_file, cmd, count = 5, existing_flag = 'kr_flag'):
        ...
        try:
            for i, doc in enumerate(documents):
                ...
                if uuid not in exist_record_dict.keys():
                    ...
                    body.append(item)
                    print(f'Kimi Processing abstract: {i + 1}/{len(documents)}')
                else:
                    print(f'Kimi Processing existed: {i + 1}/{len(documents)}')
...
if __name__ == '__main__':
    ...
    KimiAPI().batch_extract(input_file, output_file, cmd,count=10)
